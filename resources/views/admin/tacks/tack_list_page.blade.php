...
@section('title')
  ...
  <i class='subheader-icon fal fa-inbox-in'></i> Список Неисправностей

@endsection
@section('content')

  <form method="POST" name="statPage" id="statPage" action="{{route('tack_list_page')}}">
        ...
        <div class="col-12 col-md-2">
          ...
          <div class="form-group">
            ...
            <select class="form-control" name="complete" id="complete">
              ...
              <option value="3" @if ($complete=='3' ) selected @endif >Решено</option>
              <option value="5" @if ($complete=='5' ) selected @endif >Повторное исполнение</option>
              <option value="6" @if ($complete=='6' ) selected @endif >На доработке у Оператора</option>
              <option value="7" @if ($complete=='3' ) selected @endif >Консультация</option>
              <option value="8" @if ($complete=='3' ) selected @endif >Не повторилась</option>
              <option value="9" @if ($complete=='3' ) selected @endif >Не актуальная</option>
            </select>
          </div>
  ...
  <table class="dt-basic-example table table-bordered table-hover table-striped w-100" id="dt-basic-example">
    ...
    @foreach( $list AS $_k=>$_v)
      <tr id="row_{!! $_v->id !!}">
        ...
        <td>
          @if(@$_v->complete=='4'  && $_v->task_type == 'request_tack')
            <span class="badge badge-danger fw-300">Не начат</span>
          @elseif(@$_v->complete=='1'  && $_v->task_type == 'request_tack')
            <span class="badge badge-primary fw-300">Назначен Исполнитель</span>
          @elseif(@$_v->complete=='2'  && $_v->task_type == 'request_tack')
            <span class="badge badge-info fw-300">Принято в работу</span>
          @elseif(@$_v->complete=='5'  && $_v->task_type == 'request_tack')
            <span class="badge badge-warning fw-300">Повторное исполнение</span>
          @elseif(@$_v->complete=='3' && $_v->task_type == 'request_tack')
            <span class="badge badge-success fw-300">Решено</span>
          ...
          @elseif(@$_v->complete=='9' && $_v->task_type == 'request_tack')
            <span class="badge badge-warning fw-300">Не актуальна</span>
          @endif
        </td>
        <td>
          @if(@$_v->is_close==1  && $_v->task_type == 'request_tack')
